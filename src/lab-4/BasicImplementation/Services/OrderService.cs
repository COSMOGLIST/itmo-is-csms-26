...
public class OrderService : IOrderService
{
    ...
    public async Task<long> CreateAsync(Order order, CancellationToken cancellationToken)
    {
        ...
        var orderHistory = new OrderHistory(
            ...
            new CreatePayload());
        await _orderHistoryService.CreateAsync(orderHistory, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (order.OrderState == OrderState.Created)
        {
            ...
            var orderHistory = new OrderHistory(
                ...
                new ItemAddPayload(orderItemId));
            await _orderHistoryService.CreateAsync(orderHistory, cancellationToken);
    ...
    public async Task RemoveOrderItemAsync(long orderId, long orderItemId, CancellationToken cancellationToken)
    {
        ...
        if (order.OrderState == OrderState.Created)
        {
            ...
            var orderHistory = new OrderHistory(
                ...
                new ItemRemovePayload(orderItemId));
            await _orderHistoryService.CreateAsync(orderHistory, cancellationToken);
        }
    ...
    public async Task CancelOrderAsync(long id, CancellationToken cancellationToken)
    {
        ...
        {
            throw new Exception(
                "You can't cancel order this order. It has the status \""
                + order.OrderState + '\"');
        }
    ...
    public async Task CancelOrderPrivilegedAsync(long id, CancellationToken cancellationToken)
    {
        using var transaction = new TransactionScope(
            TransactionScopeOption.Required,
            new TransactionOptions { IsolationLevel = IsolationLevel.ReadCommitted },
